...
__global__ void reduce3(int *g_inData, int *g_outData)
{
...
int reduction(int *a)
{
	...
	// Execute Kernels
	reduce3 << <gridDimensions, blockDimensions, block_x*sizeof(int) >> > (d_A, d_Blocks);
	reduce3 << <gridDimensions, blockDimensions, block_x*sizeof(int) >> > (d_Blocks, d_Blocks);
	reduce3 << <gridDimensions, blockDimensions, block_x*sizeof(int) >> > (d_Blocks, d_Blocks);
	reduce3 << <gridDimensions, blockDimensions, block_x*sizeof(int) >> > (d_Blocks, d_Blocks);
